...
const StickerContainer = (props) => {
  const [basicBackgroundList, setList] = useState([]);
  const [isOpen, setIsOpen] = useState(false);
  const [myBackgroundList, setMyBackgroundList] = useState([]);
  ...
  const setMyStickers = () => {
    ...
    axiosInstance
      .get(`/stickers/users?` + paramsString.toString(), { headers }, formData)
      .then((response) => {
    ...
    <div className="sticker-container">
      ...
      <div className="sticker-list-container">
        <div className="sticker-list-title">
          <span className="user-bg">내 스티커 목록</span>
        </div>
        ...
        {myStickerList.map((sticker, index) => (
          <img
            ...
            onClick={() => props.stickertemp(sticker.searchName)}
          ></img>
        ))}

        <div className="sticker-list-title">
          <span className="user-bg">기본 스티커 목록</span>
        </div>

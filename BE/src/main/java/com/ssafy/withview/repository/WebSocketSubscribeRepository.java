...
public class WebSocketSubscribeRepository {
    ...
    public Long getUserEnterChannel(Long userSeq) {
        return Long.parseLong(Optional.ofNullable(hashOpsUserEnterChatChannelInfo.get(ENTER_CHAT_CHANNEL, String.valueOf(userSeq))).orElse("0"));
    }
    ...
    public Long userDisconnect(String simpSessionId) {
        Long userSeq = Long.parseLong(Optional.ofNullable(valOpsUserSessionInfo.get(USER_CONNECT_SESSION + simpSessionId)).orElse("0"));
        Long channelSeq = getUserEnterChannel(userSeq);
        Long serverSeq = Long.parseLong(Optional.ofNullable(valOpsUserServerInfoOfNowChannel.get(USER_ENTER_SERVER_USERSEQ + userSeq)).orElse("0"));
